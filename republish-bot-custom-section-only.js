...
(async () => {
  ...
  await cluster.task(async ({ page, data: customTemplateId }) => {
    ...
    try {
      ...
      //console.log(`https://pitch.realtair.com/custom-template/${customTemplateId}/run/edit-custom-template?returnUrl=https://pitch.realtair.com/templates`);
      await page.goto(
      ...
      console.error("Error navigating to templates page in custom template id: "+customTemplateId);
      console.error(error);
    }
    ...
    //try {
      ...
      await page.waitForSelector(reloadingLoaderCssSelector, { hidden: true, timeout: 120000 });
    ...
    if( compoCount !== 0 && compoCount !== null){
      ...
      for (let element of compoEditButtons) {
        try {
          ...
          console.error("Error in clicking the compo edit button in custom template id: "+customTemplateId);
          console.error(error);
        }
        ...
        if(await page.$(errorBtnCssSelector) !== null){
          ...
          try {
            ...
            console.error("Error in clicking the error button in custom template id: "+customTemplateId);
            console.error(error);
          }
          compoIndex++;
          continue;
        ...
        if(await page.$(iframeClassName) !== null){
          ...
          try {
            ...
            console.error("Error waiting for component frame in componentId: "+ customTemplateId);
            console.error(error);
          }
          ...
          while (!finishedFlag) {
           ...
           // }
            ...
            if (await page.$(errorBtnCssSelector) !== null) {
              ...
              //try {
                await frame.waitForSelector(loadingScreenId);
                await frame.waitForSelector(loadingScreenId, { hidden: true , timeout: 120000});
              //} catch (error) {
          ...
          //try {
            console.log("Finished submitting component number " + compoIndex + "! In custom template id: "+ customTemplateId);
            //await page.waitForSelector(updatingScreenIdName);
            //console.log("Found updating screen!");
            await page.waitForSelector(updatingScreenIdName, { hidden: true, timeout: 120000 });
    ...
    await PublishChanges(page, reloadingLoaderCssSelector, customTemplateId, errorBtnCssSelector);
    ...
    try {
      ...
      console.error("Error in final loading screen in custom template id: "+ customTemplateId);
      console.error(error);
    }
    ...
    try {
      ...
      console.error("Error in writing the custom template Id to the done file in custom template id: "+customTemplateId);
      console.error(error);
    }
  ...
  cluster.on('taskerror', (err, data, willRetry) => {
    if (willRetry) {
      ...
      fs.writeFileSync(outputFilePath, `First Error in custom template id ${data}. Error : ${err.message}\n`, {flag:'a'});
    } else {
      console.error(`Failed to crawl ${data}: ${err.message}`);
      try {
        fs.writeFileSync(outputFilePath, `Final Error in custom template id ${data}. Error : ${err.message}\n`, {flag:'a'});
      } catch (error) {
        console.error("Can't write on outputfile for customtemplate id: "+customTemplateId);
      }
    }
  });
...
async function PublishChanges(page, reloadingLoaderCssSelector, customTemplateId, errorBtnCssSelector) {
  const publishBtnCssSelector = "#page-wrapper > div > div.presentation-preview-toolbar.full-width.hidden-xs > div > div:nth-child(3) > ul > li:nth-child(3) > a";
  try {
    await Promise.all([
...
async function PublishChanges(page, reloadingLoaderCssSelector, customTemplateId) {
  const publishBtnCssSelector = "#page-wrapper > div > div.presentation-preview-toolbar.full-width.hidden-xs > div > div:nth-child(3) > ul > li:nth-child(3) > a";
  try {
    await Promise.all([
      page.waitForSelector(reloadingLoaderCssSelector, { hidden: true }),
...
async function CheckForErrorModal(page, errorBtnCssSelector, customTemplateId, frame, loadingScreenId) {
  if (await page.$(errorBtnCssSelector) !== null) {
    //console.log("error modal detected!");
    try {
      await page.click(errorBtnCssSelector);
...
async function SaveChanges(page) {
  ...
  try {
    ...
    console.error("Error in looking for the save button in custom template id: "+customTemplateId);
  }
...
async function Login(page, pitchDashboardURL) {
  ...
  if ((await page.$("#username")) !== null) {
    ...
    try {
      console.log(process.env.REALTAIR_USERNAME);
      console.log(process.env.REALTAIR_PASSWORD);
      ...
      console.error("Error in logging in custom template id: "+customTemplateId);
      console.error(error);
    }
  }
  else{
    console.log("Cannot find username field in custom template id: "+ customTemplateId);
  }
}

async function UpdateCustomTemplateSettings(page, customTemplateId, loadingScreenId, reloadingLoaderCssSelector) {
  ...
  const leftPaneCssSelector = "#page-wrapper > div > div.full-width.presentation-preview__center.presentation-preview__center--template-editor > div > div > iframe";
